...
to quickly explore panel data datasets from various sources. 
This vignette will guide you through the process of using ExPanD.
...
No worries: You data won't be stored by the server and will be deleted from memory 
once the connection to the server is closed.

## Starting ExPanD to upload a local file containing panel data

Maybe the easiest way to start using "ExPanD" is to use it with a local data file containing panel data. 
...
knitr::include_graphics("figures/ExPanD_file_01.png")
```

Now you need a file to explore. Feel free to use whatever you want but I will use the well-known `gapminder` dataset provided by the gapminder package (click [here](https://www.gapminder.org) if you do not know the gapminder initiative).
...
Use the file dialog to load the CSV file that `write.csv()` generated 
(if you are not using R, you can download the CSV file [here](https://www.dropbox.com/s/ba9gkoxsdbyzh1v/gapminder.csv?dl=1)).
...
```{r, out.width = "90%", fig.align="center", fig.border = "none", echo=FALSE}
knitr::include_graphics("figures/ExPanD_file_02.png")
```
...
Besides acting as a tool for interactive exploratory data analysis, one key objective of ExPanD is to enable readers to quickly assess the robustness of statistical inference in academic studies. For that to work, the app needs to be publicly available along with the relevant data. As shiny apps can be hosted on dedicated servers, this is easily achieved. Maybe you noticed the "Publish" icon in the top-right corner of your ExPanD window. Click on it to learn more about your options. As an example, you can access an ExPanD-based extension of a current working paper of our team [here](https://jgassen.shinyapps.io/expacc/).
