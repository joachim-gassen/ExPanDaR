...
You can customize ExPanD along several dimensions without interfering at the code level:
...
This vignette will guide you through each of these opportunities.

## Select components to include and their order

When you start ExPanD in the default mode, e.g., by
...
`grouping`               A drop down menu that gives you the option to name a 
                         grouping factor. The grouping factor will allow to 
                         focus certain components on a sub-sample defined by on 
...
`histogram`              A histogram that you can adjust to every numerical or 
                         logical variable
...
`quantile_trend_graph`   A graph that visualizew the distribution of one chosen 
                         variable over time
...
`scatter_plot`           A scatter plot, where you can use up to 4 dimension 
                         to represent your data (x, y, size, color). You can 
...
Alternatively, you can choose the components that you want to _include_. This gives you also the option to sort the components in your preferred order. Let us assume that we want to focus on the Preston curve association of GDP per capita and life expectancy in the 'gapminder' data. So maybe we want to include the following components:
...
Now tool-tips in descriptive table inform about our variables. Neat. 
...
html_blocks <- c(
  ...
  paste('<div class="col-sm-2"><h3>Transform variables</h3></div>', 
        ...
        "assess their association with life expectancy",
  ...
  paste('<div class="col-sm-2"><h3>Assess Robustness</h3></div>',
        ...
        "fixed effects.",
        "Does this also hold when you use a log-transformrd version",
        "of GDP per capita?",
        "</div>")
)
...
This is it. Now we have a (well at least somewhat) customized version of ExPanD. If you do not want to run the code yourself, you can take a quick look at the customized version [here](https://jgassen.shinyapps.io/expand_gapminder).

If you want to take customization to the next level, I would suggest forking the code of the 'ExPanDaR' package on github. The code for the app is in the "inst/app" folder of the package.
